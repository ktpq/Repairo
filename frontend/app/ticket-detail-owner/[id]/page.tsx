...
export default function TicketDetail() {
    ...
    const deleteRequest = async () => {
        Swal.fire({
            title: "คุณต้องการลบคําขอหรือไม่?",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                console.log(request_id, dorm_id)
                 try{
                    const base_api = process.env.NEXT_PUBLIC_API_URL
                    const response = await axios.put(`${base_api}/request/admin/${dorm_id}` , {id: request_id}, {withCredentials: true})
                    console.log(response.data)
                    alertSuccess("ลบคำขอสำเร็จ").then(() => {
                        window.location.href = `/owneddorm/${dorm_id}`
                    })
                } catch (error){
                    ...
                    <div className="bg-[#E2F0FF] rounded-lg py-10 px-12 relative">
                        ...
                        <div className="flex justify-end mt-6">
                            {request?.status !== "in_progress" && request?.status !== "completed" && (
                                <button className="bg-red-600 text-white px-4 py-1 rounded-lg hover:bg-red-700 transition cursor-pointer" onClick={deleteRequest}>
                                    delete
                                </button>
                            )}
                        </div>
